...
*以下内容（ 第2/3/4节 ）仅适用于 .py 文件的运行。对于打包好的 .exe 文件，不需要进行额外的配置。*
...
研究方向 ( Topics ) : OpenAlex 先依据论文之间的引用关系构建主题群，再用模型读取每篇论文的标题、摘要、引用和期刊信息，为论文分配最多 3 个主题；作者的 Topics 则汇总其论文最常出现的主题并排序。新论文被收录后，作者的主题计数和排序可能变化。
